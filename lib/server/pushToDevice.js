...
Push.Configure = serverConfig => {
  ...
  const querySend = async (query, mongoNote) => {
    ...
    await Push.appCollection.find(query).mapAsync(app => {
      if (isDebug) {
    ...
    if (isDebug) {
      ...
      if (!countApn.length && !countAndroid.length && !countWeb.length) {
        if (!await Push.appCollection.findOne()) {
        ...
        if (!await Push.appCollection.findOne({ 'token.web': { $exists: true } })) {
  ...
  if (serverConfig.defaults && serverConfig.defaults.sendInterval !== null) {
    const sendNotification = mongoNote => {
      ...
      const reserved = Push.notifications.updateAsync({
        _id: mongoNote._id,
        sent: false,
        sending: { $lt: now }
      }, {
