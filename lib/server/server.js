/* globals Mongo, Meteor, Match, Random */
...
Meteor.methods({
    ...
    if (!doc) {
      ...
      Push.appCollection.insert(doc, (err, res) => {
        if (err) {
          ...
          if (Push.debug) { console.log('Successfully inserted something in the Push.appCollection and saved to local sotrage', res) }
        }
      })
    } else {
  ...
  'push-setuser': function (pushTokenDBId) {
    ...
    var found = Push.appCollection.update({ _id: pushTokenDBId }, { $set: { userId: this.userId } }, (err, res) => {
      if (err) {
