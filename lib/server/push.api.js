...
import { sendApn, sendAndroid } from './note_constructor'

// TODO study the possibility to use Redis instead of Mongo for notifications to cut down on in/outs of Mongo.
...
Push.Configure = serverConfig => {
  ...
  */
  ...
  if (serverConfig.android) {
    if (isDebug) { console.log('Firebase Admin Messaging configured') }
  ...
  if (serverConfig.defaults && serverConfig.defaults.sendInterval !== null) {
    ...
    let sendNotification = mongoNote => {
      ...
      if (reserved) {
        var result = self.serverSend(mongoNote)
