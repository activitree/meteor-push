...
class PushHandle extends EventState {
  ...
  Configure (configuration = {}) {
    if (!this.configured) {
      ...
      console.log('PushHandle.Configure:', configuration)
        ...
        if (typeof PushNotification !== 'undefined') {
          ...
          this.push.on('registration', data => {
            ...
            if (data && data.registrationId && oldToken !== data.registrationId) {
              let token = { [getService()]: data.registrationId }
          ...
          this.push.on('notification', data => {
            ...
            console.log('PushHandle.Notification:', data)
      ...
      const initPushUpdates = appName => {
        Meteor.startup(() => {
          ...
          if (addUserId) {
            Tracker.autorun(() => {
              Meteor.userId()
              let storedTokenId = deviceStorage.getItem('pushTokenId')
